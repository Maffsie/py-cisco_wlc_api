...
def _assert(condition: bool,
            message: str,
            exception_type: BaseException = WLCAssertionException) -> bool:
    if not condition:
        raise exception_type(message)


def _is_type(in_obj, expect_type: type, message: str = None) -> bool:
    _assert(isinstance(expect_type, type), "Nested assertions? they do happen."
                                           "expect_type param must be a type.")
    _assert(isinstance(in_obj, expect_type), exception_type=IncorrectTypeError,
            message=message if message is not None else
            f"Expected an object of type {expect_type}, but input object"
...
def baseurl(url: str):
    ...
    _assert(Patterns.URLPROTO.match(url),
            f"Base URL parameter ('{url}') must begin with http(s)://")
    _assert(not url.endswith("/"),
...
def length(item, size: int, message: str = None):
    ...
    _assert(len(item) == size, exception_type=IncorrectLengthError,
            message=message if message is not None else
...
def credential(credentials: tuple):
    ...
    _is_type(credentials, tuple, "Credentials parameter must be a tuple, but "
                                 f"given object was actually {type(credentials)}")
    length(credentials, size=2, message=("Credentials parameter must contain "
